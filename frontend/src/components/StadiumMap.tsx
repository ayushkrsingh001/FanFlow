...
export default function StadiumMap({ isOpen = true, onClose, route, stepFree }: StadiumMapProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          ...
          <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
            <div className="bg-white border border-[#E2E8F0] px-3 py-1.5 rounded-md shadow-sm text-xs font-semibold text-[#0F172A] inline-block w-max">
              Venue Map Preview
            </div>
            {stepFree && (
              <div className="bg-[#EFF6FF] border border-[#BFDBFE] px-2 py-1 rounded text-xs font-medium text-[#1D4ED8] flex items-center gap-1 inline-block w-max">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 1 0 0-20z"/><path d="M12 8v4l3 3"/></svg>
                Step-Free Route
              </div>
            )}
          </div>
          ...
          <div className="w-full h-full relative flex items-center justify-center">
            ...
            <svg viewBox="0 0 400 200" className="w-full h-full opacity-80">
              ...
              {/* Zones */}
              <text x="50" y="105" fill="#64748B" fontSize="10" fontWeight="600">{route ? route[0] : 'Gate W'}</text>
            ...
            >
              ...
              <span className="text-[10px] font-bold bg-white border border-[#E2E8F0] shadow-sm px-1.5 py-0.5 rounded text-[#0F172A] mt-1">
                {route && route.length > 1 ? route[1] : 'Sec 112'}
              </span>
            </motion.div>
          </div>
        </motion.div>
